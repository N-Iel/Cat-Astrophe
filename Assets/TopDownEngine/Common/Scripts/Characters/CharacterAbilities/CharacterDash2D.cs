...
namespace MoreMountains.TopDownEngine
{
	...
	public class CharacterDash2D : CharacterAbility 
	{
        ...
        /// the duration of the time between the end of the dash and the player recover control
        [Tooltip("the duration between the end of the dash and recover movement")]
        public float DashBlockDuration = 0f;
		...
		public virtual void DashStop()
		{
			...
			_dashing = false;
            if (DashBlockDuration > 0)
            {
                Invoke("UnblockMovement", DashBlockDuration);
            }
            else
            {
